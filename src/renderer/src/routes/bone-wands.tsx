...
const wandStats: Array<{ label: string; getValue: (wand: Wand) => string }> = [
  ...
  { label: 'Cast delay', getValue: (wand) => `${formatNumber(wand.castDelay)} s` },
  { label: 'Rechrg. Time', getValue: (wand) => `${formatNumber(wand.rechargeTime)} s` },
  { label: 'Mana max', getValue: (wand) => formatNumber(wand.manaMax) },
  { label: 'Mana chg. Spd', getValue: (wand) => formatNumber(wand.manaChargeSpeed) },
...
function BoneWandsPage() {
  ...
  const sortedBoneWands = useMemo(
    () => [...boneWands].sort((left, right) => Date.parse(right.lastModified) - Date.parse(left.lastModified)),
    [boneWands],
  );
  const selectedBoneWand = useMemo(() => {
    const selected = selectedFileName
      ? sortedBoneWands.find((boneWand) => boneWand.fileName === selectedFileName)
      : undefined;
        ...
        <ResizablePanel defaultSize={36} minSize={24} maxSize={55}>
          <section className="flex h-full flex-col border-r">
            <div className="flex h-12 items-center justify-between border-b px-4">
              ...
              <div className="text-xs text-muted-foreground tabular-nums">{sortedBoneWands.length}</div>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {sortedBoneWands.map((boneWand) => {
                    const selected = selectedBoneWand?.fileName === boneWand.fileName;
                    return (
                      <TableRow
                      ...
                      >
                        <TableCell className="font-medium">{boneWand.fileName}</TableCell>
                        <TableCell>{formatDate(boneWand.lastModified)}</TableCell>
                      </TableRow>
                    );
                  })}
  ...
  if (!boneWand) {
    return (
      <aside className="flex h-full flex-col">
        <div className="flex h-12 items-center border-b px-4 text-xs font-medium">No wand selected</div>
        <div className="p-4 text-xs text-muted-foreground">Select a bone wand.</div>
      </aside>
    );
  }
  ...
  return (
    <aside className="flex h-full flex-col">
      <div className="flex h-12 items-center justify-between border-b px-4">
        ...
        <div className="text-xs text-muted-foreground">{formatDate(boneWand.lastModified)}</div>
      </div>
        ...
        <div className="grid gap-5">
          <section className="grid gap-3">
            ...
            <div className="flex min-h-28 items-center justify-center rounded-md border bg-muted/30 p-3">
              {wand.spriteFileUrl ? (
                ...
                <div className="text-xs text-muted-foreground">No wand image</div>
              )}
            </div>
          </section>
          ...
          <section className="grid gap-2">
            ...
            <div className="grid max-w-lg grid-cols-[10rem_1fr] border text-xs">
              {wandStats.map((stat) => (
                <StatRow key={stat.label} label={stat.label} value={stat.getValue(wand)} />
              ))}
            </div>
          </section>

          <SpellSection title="always casts" spells={wand.alwaysCasts} />
          <SpellSection title="spells" spells={wand.spells} capacity={wand.capacity} />
        </div>
      </div>
    </aside>
  );
...
function StatRow({ label, value }: { label: string; value: string }) {
  return (
    <>
      <div className="border-r border-b px-2 py-1.5 text-muted-foreground">{label}</div>
      <div className="border-b px-2 py-1.5 tabular-nums">{value}</div>
    </>
  );
}

function SpellSection({ title, spells, capacity }: { title: string; spells: Spell[]; capacity?: number }) {
  const slotCount = capacity === undefined ? spells.length : Math.max(capacity, spells.length);

  return (
    <section className="grid gap-2">
      <div className="flex items-center justify-between">
        <div className="text-xs font-medium">{title}</div>
        <div className="text-xs text-muted-foreground tabular-nums">{spells.length}</div>
      </div>
...
function SpellSlot({ spell }: { spell: Spell | undefined }) {
  ...
  return (
    ...
    >
      {imageUrl ? (
        <img src={imageUrl} alt={label} className="max-h-8 w-auto object-contain" />
      ) : (
        <div className="px-1 text-center text-[9px] leading-tight text-muted-foreground">{spell.id}</div>
      )}
    </div>
  );
}
