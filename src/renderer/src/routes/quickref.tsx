...
const detailFields: Array<{ label: string; getValue: (spell: Spell) => string }> = [
  ...
  { label: 'spawn_probability', getValue: (spell) => spell.spawn_probability.join(', ') },
  ...
  { label: 'never_unlimited', getValue: (spell) => formatBoolean(spell.never_unlimited) },
  { label: 'spawn_manual_unlock', getValue: (spell) => formatBoolean(spell.spawn_manual_unlock) },
  { label: 'recursive', getValue: (spell) => formatBoolean(spell.recursive) },
  { label: 'ai_never_uses', getValue: (spell) => formatBoolean(spell.ai_never_uses) },
  { label: 'is_dangerous_blast', getValue: (spell) => formatBoolean(spell.is_dangerous_blast) },
  { label: 'sprite', getValue: (spell) => spell.sprite },
  { label: 'spriteUrl', getValue: (spell) => spell.spriteUrl },
  { label: 'sprite_unidentified', getValue: (spell) => spell.sprite_unidentified },
  { label: 'spriteUnidentifiedUrl', getValue: (spell) => spell.spriteUnidentifiedUrl },
  { label: 'related_projectiles', getValue: (spell) => spell.related_projectiles },
  { label: 'custom_xml_file', getValue: (spell) => spell.custom_xml_file },
  { label: 'spawn_requires_flag', getValue: (spell) => spell.spawn_requires_flag },
  { label: 'sound_loop_tag', getValue: (spell) => spell.sound_loop_tag },
  { label: 'related_extra_entities', getValue: (spell) => spell.related_extra_entities },
...
function QuickrefPage() {
  ...
  const spellImageUrl = selectedSpell?.uiImageUrl || selectedSpell?.spriteUrl || selectedSpell?.spriteUnidentifiedUrl || '';
                  ...
                  {filteredSpells.map((spell) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="font-mono text-[11px]">{spell.id}</TableCell>
                        <TableCell className="font-medium">{spell.english_name || spell.name}</TableCell>
...
function SpellDetails({ spell, imageUrl }: { spell: Spell | null; imageUrl: string }) {
  return (
    <aside className="flex h-full flex-col">
      <div className="flex h-12 items-center justify-between border-b px-4">
        <div className="truncate text-xs font-medium">{spell?.english_name ?? 'No spell selected'}</div>
        {spell && <div className="text-xs text-muted-foreground">{spell.type}</div>}
      </div>

      <div className="min-h-0 flex-1 overflow-auto p-4">
        {!spell && <div className="text-xs text-muted-foreground">Select a spell.</div>}
          ...
          <div className="grid gap-3">
            <div className="grid gap-1">
              ...
              <div className="flex min-h-20 items-center justify-center rounded-md border bg-muted/30 p-3">
                {imageUrl ? (
                  <img src={imageUrl} alt={spell.english_name || spell.name} className="max-h-16 w-auto object-contain" />
                ) : (
            ...
            {detailFields.map((field) => (
              <DetailRow key={field.label} label={field.label} value={field.getValue(spell)} />
            ))}
            <div className="grid gap-1">
...
function SpellImage({ spell }: { spell: Spell }) {
  const imageUrl = spell.uiImageUrl || spell.spriteUrl || spell.spriteUnidentifiedUrl;
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-1 border-b pb-2 last:border-b-0">
      ...
      <div className={cn('text-xs leading-relaxed text-muted-foreground', value && 'text-foreground')}>
        {value || '-'}
      </div>
    </div>
  );
...
function filterSpells(spells: Spell[], search: string) {
  ...
  return spells.filter((spell) =>
    [spell.id, spell.name, spell.description, spell.english_name, spell.english_desc]
      .join(' ')
      .toLocaleLowerCase()
      .includes(query),
  );
