...
function safeBackupName(name: string): string {
  ...
  if (trimmed !== basename(trimmed) || trimmed.includes('/') || trimmed.includes('\\')) {
...
export async function listBackups(): Promise<BackupEntry[]> {
  ...
  const entries = await readdir(config.noitaBackupFolder, { withFileTypes: true });
  const zipFiles = entries
    .filter((entry) => entry.isFile() && entry.name.toLowerCase().endsWith('.zip'))
    .map((entry) => join(config.noitaBackupFolder, entry.name));
  const backups = await Promise.all(zipFiles.map(toBackupEntry));
  return backups.sort((a, b) => Date.parse(b.lastModified) - Date.parse(a.lastModified));
}

export async function createBackup(name = defaultBackupName()): Promise<BackupEntry> {
  const config = await loadConfig();
...
export function assertZipEntryInsideDestination(destFolder: string, entryName: string): string {
...
async function extractZipSafely(zipFile: string, destFolder: string): Promise<void> {
...
export async function restoreBackup(name: string, backupFirst = false): Promise<void> {
  const config = await loadConfig();
  ...
  try {
    ...
    await rm(currentSave, { recursive: true, force: true }).catch(() => undefined);
